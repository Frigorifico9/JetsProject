...
int main() 
{
  ...
  for (int f = 0; f < filas; ++f)
  {
    ...
    {
      all_histos[f][c] = (TH2D*) mydata->Get(Form("h%d%d",f,c));
      //hist_final[f][c] = new TH1D( Form("hist_final_mult>=%d_Aj%s",mult_range[f],Aj_range[c]), "Projected pT vs Delta", 9, 0, 1.8);
