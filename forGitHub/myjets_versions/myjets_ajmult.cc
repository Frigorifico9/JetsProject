...
int main() 
{
  ...
  TH2D*  hist_empty = new TH2D("hist_empty", "The momentum of particles in each Delta Ring", 9, 0, 1.8, 1000, -20, 20);
  ...
  double pTll           = 0.0;
  double Aj             = 0;
  double mult_range[]   = {234, 204, 188, 168, 152, 140, 128, 116, 104, 92, 76}; //multiplicity ranges
  int    mult           = 0;
  ...
  //We need to keep track of the histograms we are gonna make
  TObjArray* all_histos    = new TObjArray(12); //and we are going to do it with an array of objects
  //Every multiplicity range will have its collection of histograms
  TObjArray* mult_empty    = new TObjArray(3);
  TObjArray* mult1_histos  = new TObjArray(3);
  TObjArray* mult2_histos  = new TObjArray(3);
  TObjArray* mult3_histos  = new TObjArray(3);
  TObjArray* mult4_histos  = new TObjArray(3);
  TObjArray* mult5_histos  = new TObjArray(3);
  TObjArray* mult6_histos  = new TObjArray(3);
  TObjArray* mult7_histos  = new TObjArray(3);
  TObjArray* mult8_histos  = new TObjArray(3);
  TObjArray* mult9_histos  = new TObjArray(3);
  TObjArray* mult10_histos = new TObjArray(3);
  TObjArray* mult11_histos = new TObjArray(3);
  TObjArray* mult12_histos = new TObjArray(3);

  //and we will collect every collection in the main Collection
  all_histos->Add(mult1_histos);
  all_histos->Add(mult2_histos);
  all_histos->Add(mult3_histos);
  all_histos->Add(mult4_histos);
  all_histos->Add(mult5_histos);
  all_histos->Add(mult6_histos);
  all_histos->Add(mult7_histos);
  all_histos->Add(mult8_histos);
  all_histos->Add(mult9_histos);
  all_histos->Add(mult10_histos);
  all_histos->Add(mult11_histos);
...
//####################################################################################################
    ...
    for (int n = 0; n < 12; ++n)
    {
      ...
      {
        ((TObjArray *) all_histos->At(n))->Add(hist_pTdelta_AjInc);
        ((TObjArray *) all_histos->At(n))->Add(hist_pTdelta_AjA);
        ((TObjArray *) all_histos->At(n))->Add(hist_pTdelta_AjB);
        break;
      }
    }
  // End of event loop.
  ...
  for (int i = 0; i < 12; ++i)
  {
    int N = ((TObjArray *) all_histos->At(n))->GetEntries();
    if ( (N%3) != 0 )
    {
      cout<<"Something went terribly wrong, most likely you need to simulate more events"<<endl;
    }
    for (int n = 0; n < N; ++n)
    {
      /* code */
